var spinner = {};
spinner.el = null;
spinner.size = 25;
spinner.init = function() {
	var alertHeight = this.size;
	var clientHeight = document.documentElement.clientHeight || document.body.clientHeight;

	var alertWidth = this.size;
	var clientWidth = document.documentElement.clientWidth || document.body.clientWidth;

	var div = document.createElement('div');
	div.innerHTML = '<style>@-webkit-keyframes rotate {' +
		'from {-webkit-transform:rotate(0deg);}' +
		'to {-webkit-transform:rotate(360deg);}' +
		'}</style>';
	///div.setAttribute('id', 'position: fixed; z-index: 99; left: 0; top: 0; width: 38px; height: 38px; background-image: url(../img/loading-logo.png)'); // and make sure myclass has some styles in css
	div.id = 'J_loading';
	div.style.position = 'fixed';
	div.style['z-index'] = 99;
	div.style['height'] = this.size + 'px';
	div.style['width'] = this.size + 'px';
	div.style['top'] = ((clientHeight - alertHeight) / 2) + 'px';
	div.style['left'] = ((clientWidth - alertWidth) / 2) + 'px';
	div.style['display'] = 'none';
	div.style['background-image'] = 'url(data:image/png;base64,)';
	div.style['background-size'] = this.size + 'px ' + this.size + 'px';

	div.innerHTML = '';
	document.body.appendChild(div);
	var i = document.createElement('i');
	i.style.position = 'fixed';
	i.style['z-index'] = 99;
	i.style['height'] = this.size + 'px';
	i.style['width'] = this.size + 'px';
	i.style['top'] = ((clientHeight - alertHeight) / 2) + 'px';
	i.style['left'] = ((clientWidth - alertWidth) / 2) + 'px';
	i.style['-webkit-animation'] = 'rotate 1s linear infinite';
	i.style['background-image'] = 'url(data:image/png;base64,)';
	i.style['background-size'] = this.size + 'px ' + this.size + 'px';

	this.el = document.getElementById('J_loading');
	this.el.appendChild(i);

}
spinner.start = function() {
	this.el.style.display = 'block';
}
spinner.stop = function() {
	this.el.style.display = 'none';
}
spinner.init();
spinner.start();